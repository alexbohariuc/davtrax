...
if ( ! function_exists( 'davtrax_get_the_comments_pagination' ) ) :
	function davtrax_get_the_comments_pagination( $args = array() ) {
		$navigation = '';
		$args = wp_parse_args( $args, array(
			'prev_text'				=> __( '&laquo;', 'davtrax' ),
			'next_text'				=> __( '&raquo;', 'davtrax' ),
			'size'					=> 'default',
			'show_disabled'			=> true,
		) );
		$args['type'] = 'array';
...
// Custom Comments Pagination.
if ( ! function_exists( 'davtrax_the_comments_pagination' ) ) :
	function davtrax_the_comments_pagination( $args = array() ) {
		echo davtrax_get_the_comments_pagination( $args );
	}
endif;
...
if ( ! function_exists( 'davtrax_menu_fallback' ) ) :
	function davtrax_menu_fallback() {
		echo '<div class="alert-box secondary">';
		/* translators: %1$s: link to menus, %2$s: link to customize. */
		printf(
			__( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.', 'davtrax' ),
			/* translators: %s: menu url */
			sprintf(
				__( '<a href="%s">Menus</a>', 'davtrax' ),
				get_admin_url( get_current_blog_id(), 'nav-menus.php' )
			),
			/* translators: %s: customize url */
			sprintf(
				__( '<a href="%s">Customize</a>', 'davtrax' ),
...
if ( ! function_exists( 'davtrax_active_nav_class' ) ) :
	function davtrax_active_nav_class( $classes, $item ) {
		if ( $item->current == 1 || $item->current_item_ancestor == true ) {
			$classes[] = 'is-active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'davtrax_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'davtrax_active_list_pages_class' ) ) :
	function davtrax_active_list_pages_class( $input ) {
	...
	add_filter( 'wp_list_pages', 'davtrax_active_list_pages_class', 10, 2 );
endif;
...
if ( ! function_exists( 'davtrax_mobile_menu_id' ) ) :
	function davtrax_mobile_menu_id() {
...
if ( ! function_exists( 'davtrax_title_bar_responsive_toggle' ) ) :
	function davtrax_title_bar_responsive_toggle() {
 ...
 */
if ( ! function_exists( 'davtrax_gallery' ) ) :
	function davtrax_gallery($attr) {
	...
	add_shortcode('gallery', 'davtrax_gallery');
endif;
